  ...
  async function getModeForDomain(domain) {
    const result = await extensionApi.storage.local.get(['fpGlobalMode', 'fpSiteModes']);
  ...
  async function setModeForDomain(domain, mode) {
    const result = await extensionApi.storage.local.get(['fpSiteModes']);
    const siteModes = result.fpSiteModes || {};
    if (mode === null) {
    ...
    await extensionApi.storage.local.set({ fpSiteModes: siteModes });
    ...
    const effectiveMode = mode === null
      ? ((await extensionApi.storage.local.get(['fpGlobalMode'])).fpGlobalMode || 'detect')
      : mode;
  ...
  async function setGlobalMode(mode) {
    await extensionApi.storage.local.set({ fpGlobalMode: mode });
    await updateHeaderRules();
  }
  ...
  extensionApi.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg.type === 'get_mode') {
      extensionApi.storage.local.get(['fpGlobalMode', 'fpSiteModes'], (result) => {
        const globalMode = result.fpGlobalMode || 'detect';
        const siteModes = result.fpSiteModes || {};
